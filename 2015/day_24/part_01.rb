...
target_weight = packages.inject(:+) / 3

solutions = (1..packages.length - 3).flat_map { |size| packages.combination(size).select { |s| s.inject(:+) == target_weight }.sort_by { |s| s.length } }
